...
print(bootcampStudent.purpose) // but all of bootcamp students have same purpose
...
/*
 The problem is that no event is sent when click on the selected segment. To check, added to viewDidLoad an action to track any event 🔻
 segControl.addTarget(self, action: #selector(ViewController.changeAndRepeat(_:)), forControlEvents:.AllEvents)
 
 The function 🔻
 func changeAndRepeat(sender: UISegmentedControl) {
 Swift.print("Touch ", sender.selectedSegmentIndex)
 }
 */
